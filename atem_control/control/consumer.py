...
class ATEMConsumer(ATEMConnectionLoggingMixin, ATEMStateMixin, AsyncWebsocketConsumer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Seconds of operator silence before the session is dropped, or None
        # for never (the default — see ``hooks.idle_disconnect_seconds``).
        # Read once per session in start_monitoring.
    ...
    async def connect(self):
        ...
        user = self.scope.get('user') if getattr(self, 'scope', None) else None
        # Off the event loop: a host's gate reads its role tables (ORM), which
        # Django refuses from async code.
        if not await sync_to_async(hooks.get().can_use)(user):
            await self.close()
            return
        await self.accept()
    ...
    async def _perform_disconnect(self, reason='user_requested'):
        ...
        if reason == 'inactivity_timeout':
            minutes = max(1, round((self.idle_timeout or 0) / 60))
            message = f'Auto-disconnected due to inactivity ({minutes} minutes)'
    ...
    async def start_monitoring(self):
        ...
        self.idle_timeout = hooks.get().idle_disconnect_seconds()
    ...
    async def _monitor_loop(self):
        ...
        Drops the session after ``self.idle_timeout`` seconds of operator
        silence when the host asked for that (never, by default).
        ...
        try:
            ...
            while self.is_monitoring:
                ...
                # Idle disconnect — only if the host asked for one.
                current_time = asyncio.get_event_loop().time()
                if self.idle_timeout and self.last_command_time > 0:
